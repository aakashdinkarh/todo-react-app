...
import { TodosList, Button } from '../Home.styled';
import { useCallback } from 'react';
import { saveTodosToLocalStorage } from '../../utils/localStorageUtil';
import { TRenderTodoList, TTodo, TToDoList } from '../types';
import { getTodosSorted } from '../../utils/getTodosSorted';

const todoElementHeight = 30.5;
const todoElementMargin = 10;
const translateY = todoElementHeight + todoElementMargin;
...
const RenderTodoList = ({
	...
	const todosRef = useRef<Array<HTMLParagraphElement | null>>([]);
	const sortedTodos = getTodosSorted(todos);
	const translateYForTodos = Array.from({ length: sortedTodos.length }, (_, index) => `translateY(${index * translateY}px)`);

	return (
		<>
			{sortedTodos.map((todo, index) => (
				<p className='todo-item' key={todo.id} ref={r => { todosRef.current[index] = r }} style={{ transform: translateYForTodos[index] }}>
					<span>{index + 1}. </span>
					...
					<Button className='editBtn' name='edit-todo' onClick={() => handleEdit(todo)}>
						<svg
					...
					<Button className='delBtn' name='delete-todo' onClick={() => deleteToDo(todo.id)}>
						&times;
					</Button>
				</p>
			))}
...
export const ToDoList = ({ searchTerm, todos, setToDos }: TToDoList) => {
	...
	const todoListHeight = ((todosToShow.length + 1) * translateY) + todoElementMargin;

	return (
		<TodosList style={{ height: `${todoListHeight}px` }}>
			<RenderTodoList
				todos={todosToShow}
